...
| M4. 마이 페이지와 로컬 사용자 정보 | 6 / 6 | 로그인 기능은 추후 확장으로 축소. 로컬 프로필, D3 대시보드, Goals 요약 차트, 월 이동 달력 구현 완료 |
...
- 마이 페이지를 로컬 프로필, D3 러닝 대시보드, Goals 요약 차트, 월 이동 러닝 달력, 앱 설정, 위치 권한 안내 중심의 독립 페이지로 구성했다.
...
- 구현 기준 완료. Goals 요약 차트와 월 이동 달력을 추가했다. 추후에는 목표 입력 UI, 그래프 라벨·색상·간격, empty state, 기록 리셋, 평균 페이스 추이 그래프를 polish 작업으로 검토한다.
