...
Polling In Run은 현재 지도 기반 편의시설 탐색, 러닝 기록 MVP, ID/PW 인증 화면, Supabase Auth 클라이언트 연결, 인증 실패 메시지 정리까지 완료한 상태다.
...
| M4. 회원가입과 로그인 | 5 / 8 | ID/PW 화면, Supabase Auth 클라이언트, 기록 접근 로그인 게이트, 인증 실패 메시지 정리 완료 |
