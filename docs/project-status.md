...
Polling In Run은 현재 지도 기반 편의시설 탐색, 러닝 기록 MVP, 로컬 대시보드, 기록 페이지 확장, Capacitor iOS Preview 기본 설정까지 완료했고, 방향을 local-first iPhone prototype으로 재정의한 상태다.

실제 모바일 실사용 검증, NativeMap Capacitor plugin 초안 추가, 시설 JSON 로딩 성능 확인이 남아 있다.
...
| M5. Local-first iPhone 프로토타입 | 7 / 8 | Capacitor 설정 완료. 시설 데이터 로컬 JSON 생성, Apple MapKit native 구현 경로 문서화 |
...
1. Xcode에서 `NativeMap` Capacitor plugin과 Swift `MKMapView` 화면 초안을 추가한다.
...
- NativeMap Capacitor plugin 초안을 추가한다.
