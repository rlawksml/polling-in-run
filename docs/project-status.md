...
Polling In Run은 현재 지도 기반 편의시설 탐색, 러닝 기록 MVP, 로컬 대시보드, 기록 페이지 확장, Capacitor iOS Preview 기본 설정, NativeMap iOS 화면 초안까지 완료했고, 방향을 local-first iPhone prototype으로 재정의한 상태다.

실제 모바일 실사용 검증, NativeMap 실기기 동작 확인, 시설 JSON 로딩 성능 확인이 남아 있다.
...
| M5. Local-first iPhone 프로토타입 | 7.5 / 8 | Capacitor 설정 완료. 시설 데이터 로컬 JSON 생성, NativeMap iOS 화면 초안 구현 |
...
- Apple MapKit native는 Capacitor plugin이 Swift 지도 화면을 별도로 여는 방식으로 시작한다.
- NativeMap iOS 초안은 React에서 현재 위치와 최대 300개 시설을 넘기고, Swift `MKMapView` 화면에서 annotation으로 표시한다.
...
- 기록 탭에서 메모 있음 필터, 월별 필터, 거리·시간·페이스 정렬을 사용할 수 있다.
- 새로 저장한 기록은 경로 좌표를 함께 저장하고, 상세에서 SVG 경로 미리보기를 제공한다.
...
- 마이 페이지를 로컬 프로필, D3 러닝 대시보드, 앱 설정, 위치 권한 안내 중심으로 구성했다.
...
1. iPhone 실기기에서 `NativeMap` Capacitor plugin과 Swift `MKMapView` 화면 초안을 확인한다.
