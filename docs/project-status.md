...
실제 모바일 실사용 검증과 NativeMap 실기기 동작 확인이 남아 있다.
...
- 앱 초기 위치 확인 중에는 전체 랜딩 로딩 화면을 표시하고, 시설 데이터 로딩 중에는 지도 위 스켈레톤을 표시한다.
...
- 시설 JSON은 약 5.2MB이고 gzip 기준 약 389KB다. 로컬 Node 기준 `JSON.parse` 평균은 약 8.7ms였다.
...
1. iPhone 실기기 홈 화면에서 Apple MapKit 배경 지도, 현재 위치, 시설 annotation, 지도 드래그·확대/축소, 현위치 이동 버튼을 확인한다.
2. 모바일 브라우저와 Capacitor iOS 실기기 검증 체크리스트를 수행한다.
3. 시설 JSON 로딩 체감을 iPhone 실기기에서 확인한다.
...
- 시설 JSON 크기와 로컬 파싱 성능은 1차 확인했다. iPhone에서 체감 로딩이 길면 gzip 서빙, 필드 축소, 지역별 분할 순서로 검토한다.
