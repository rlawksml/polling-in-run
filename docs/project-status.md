...
- 새로 저장한 기록은 경로 좌표를 함께 저장하고, 상세에서 카드 내부 SVG 경로를 제공한다.
...
- iPhone 실기기에서 위치 권한, 현재 위치 마커, 시설 마커, Apple MapKit 드래그·확대/축소, 현위치 이동 버튼을 확인했다. 기록 상세 경로는 native overlay 대신 카드 내부 SVG로 정리했고, 기록 탭은 독립 페이지로 전환해 지도 배경과 레이어 충돌을 줄였다.
