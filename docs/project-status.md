...
- 새로 저장한 기록은 경로 좌표를 함께 저장하고, 상세에서 웹 SVG 또는 iPhone MapKit snapshot 기반 경로 미리보기를 제공한다.
...
- iPhone 실기기에서 위치 권한, 현재 위치 마커, 시설 마커, Apple MapKit 드래그·확대/축소, 현위치 이동 버튼을 확인했다. 기록 상세 경로는 live map view 대신 snapshot 이미지 위에 GPS 경로를 그리는 방식으로 정리했다.
