...
Polling In Run은 현재 지도 기반 편의시설 탐색, 러닝 기록 MVP, ID/PW 인증 화면과 Supabase Auth 클라이언트 연결까지 완료한 상태다.
...
| M4. 회원가입과 로그인 | 4 / 8 | ID/PW 화면, Supabase Auth 클라이언트, 기록 접근 로그인 게이트 준비 완료 |
...
- 기록은 현재 `localStorage`의 `polling-in-run.records.v1`에 저장한다.
...
3. ID 중복 확인과 인증 실패 메시지를 사용자 친화적으로 정리한다.

### M4 남은 범위

- ID 중복 확인
- 로그인한 사용자별 러닝 기록 분리
- 인증 실패, 중복 ID, 입력 오류 상태 처리 고도화
- 회원 탈퇴
...
- 사용자별 기록 분리는 Supabase DB 연결 이후에 완료된다.
