...
- Apple MapKit native는 React 화면 안에 직접 끼우기보다 Capacitor plugin이 Swift 지도 화면을 별도로 여는 방식으로 시작한다.
...
- [ ] Xcode에서 NativeMap plugin과 Swift 지도 화면을 추가한다.
- [ ] 현재 위치 권한 요청이 표시된다.
- [ ] 현재 위치 마커가 표시된다.
- [ ] 시설 API가 `VITE_API_BASE_URL`을 통해 호출된다.
- [ ] 음수대와 화장실 마커 또는 클러스터가 표시된다.
- [ ] Native MapKit 화면에서 현재 위치와 시설 annotation이 표시된다.
...
- Apple MapKit native Capacitor plugin 초안을 Xcode 프로젝트에 추가한다.
