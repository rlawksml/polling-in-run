...
describe('App', () => {
  ...
  it('shows local profile, record summary, and settings on My page', async () => {
    ...
    expect(screen.getByText('이번 주 차트')).toBeInTheDocument()
    fireEvent.click(screen.getByRole('button', { name: /월간 목표 차트 보기 40 km/ }))
    expect(screen.getByText('월간 목표 차트')).toBeInTheDocument()
