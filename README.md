...
Kakao Maps 기반 홈 화면에서 현재 위치, 지도 컨트롤, 실제 서울 공원 음수대 마커, 실제 서울 공중화장실 마커를 표시합니다. FastAPI는 서울 열린데이터광장 음수대 데이터와 공간데이터마켓 화장실 DBF 원본을 공통 `Facility` 모델로 정규화하고, 현재 지도 영역과 현재 위치 반경 안의 시설을 거리순으로 반환합니다.

마커를 선택하면 시설명, 주소, 거리 정보를 확인할 수 있으며 카카오맵 길찾기로 이동할 수 있습니다. 러닝 프로세스는 `idle → running → paused → finished` 상태 모델과 1차 진행 화면까지 구현했습니다. 다음 큰 작업은 GPS 기반 이동 경로 추적과 마커 클러스터링입니다.
