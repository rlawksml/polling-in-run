...
- [x] 마이 페이지 러닝 대시보드 1차 구현: 목표 설정, 클릭형 Goals 차트, 월 이동 달력
...
마커를 선택하면 시설명, 주소, 거리 정보를 확인할 수 있으며 외부 지도 길찾기로 이동할 수 있습니다. 러닝 중에도 홈 지도에서 시설 필터, 현재 영역 재검색, 현재 위치 이동을 사용할 수 있습니다. iPhone 앱에서는 현재 위치를 기본 파란 점 대신 러너 아이콘에 가까운 MapKit SF Symbol로 표시합니다. 러닝 프로세스는 `idle → running → paused → finished` 상태 모델, iOS native background location 브릿지, GPS 기반 거리·페이스 계산, 러닝 결과 확인 화면, 로컬 기록·메모 저장과 기록 목록/상세·삭제까지 구현했습니다. 기록 페이지는 지도 위 floating panel이 아니라 독립 페이지로 표시하며, debounce가 적용된 메모 검색, 월별 필터, 거리·시간·페이스 정렬, 경로 표시를 제공합니다. iPhone 앱에서는 Apple MapKit snapshot 이미지 위에 저장된 GPS 경로를 그려 카드 내부 이미지로 표시하고, 웹/실패 시에는 SVG 경로로 fallback합니다. 이후 방향은 서버 기능을 더 늘리기보다, 내 iPhone에서 안정적으로 돌아가는 local-first 프로토타입으로 범위를 줄이는 것입니다. 마이 페이지는 로그인보다 로컬 러닝 대시보드에 집중하며, 총 거리, 횟수, 클릭형 Goals 차트, D3 기반 월간 거리·목표 비교 그래프, 월 이동 러닝 날짜 달력을 보여줍니다. 인증·동기화는 추후 확장으로 분리합니다.

> iOS 백그라운드 러닝 추적은 네이티브 코드와 권한 문구가 추가된 상태입니다. 실제 화면 잠금·앱 전환 중 기록 안정성은 Xcode 실기기에서 위치 권한을 `항상 허용`으로 설정한 뒤 검증해야 합니다.
