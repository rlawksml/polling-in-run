...
ID 중복 확인과 회원 탈퇴는 FastAPI가 Supabase Admin API를 호출하므로 `apps/api/.env`에도 아래 값을 추가합니다. `SUPABASE_SERVICE_ROLE_KEY`는 절대 프론트엔드 환경변수에 넣지 않습니다.
