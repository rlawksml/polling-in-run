...
| M2. 주변 편의시설 | 7 / 12 | 실제 서울 음수대 API 수집·정규화와 거리순 정렬 완료 |
| M3. 러닝 기록 | 0 / 5 | 구현 전 |
| M4. 회원가입과 로그인 | 0 / 8 | 요구사항 정리 완료, 구현 전 |
| 기반 작업 | 6 / 6 | React, FastAPI, 테스트, shadcn/ui + Tailwind 기반 완료 |

현재 앱은 카카오맵에서 실제 서울 공원 음수대와 샘플 화장실을 표시하며, 위치 권한을
허용하면 FastAPI가 현재 위치 반경 안의 시설을 거리순으로 가공해 반환한다. 화장실은
아직 실제 공공데이터를 연결하지 않았으므로 검증용 샘플이다.

### 바로 다음 작업

1. 모바일 브라우저에서 실제 위치와 주변 시설 요청 확인
2. 실제 음수대 마커 모바일 표시 확인
3. 화장실 실제 데이터 확보와 정규화
4. 시설 상세 정보 UI 구현
