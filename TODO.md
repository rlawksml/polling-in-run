...
| M4. 회원가입과 로그인 | 2 / 8 | ID/PW 회원가입·로그인 화면과 입력 검증 안내 1차 완료 |
...
1. M4 Supabase Auth 연결 방식 결정과 환경변수 준비
2. ID 중복 확인, 로그인 상태 유지, 로그아웃 구현
...
- 실제 회원가입, 로그인, 세션 유지는 Supabase Auth 연결 PR에서 구현한다.
