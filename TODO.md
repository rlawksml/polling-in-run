...
| M3. 러닝 기록 | 6 / 6 | 러닝 상태, 위치 추적, 거리·페이스, 결과 확인, 로컬 기록 저장·조회 완료 |
| M4. 마이 페이지와 로컬 사용자 정보 | 8 / 8 | 로그인 기능은 추후 확장으로 축소, 로컬 프로필·러닝 대시보드·설정 안내 구현 완료 |
| M5. Local-first iPhone 프로토타입 | 15 / 15 | Capacitor 설정, Apple MapKit 대체, native 위치 브릿지, 기록 snapshot 안정화 완료 |
...
1. iPhone QA를 계속 진행한다: 기록 빠른 클릭, 경로 snapshot, GNB 아이콘 터치감, Goals 차트 전환, 백그라운드 위치 추적.
2. QA에서 발견한 불편함은 GitHub Issue로 분리해 작은 PR 단위로 처리한다.
3. PWA 설치 경험과 Capacitor 앱 경험을 비교한다.
4. 접근성·성능·배포 점검과 포트폴리오 회고를 준비한다.
...
- [x] 러닝 결과 확인 화면 구현
- [x] 기록과 메모 저장
- [x] 0km 기록 저장 차단과 빈 메모 기록 저장 차단

완료 조건:

- 사용자가 러닝을 시작하고 종료한 뒤 거리, 시간, 메모를 저장할 수 있다.
- 0km 러닝은 저장하지 않고, 메모가 비어 있으면 기록 자체를 저장하지 않는다.
...
- [x] D3 기반 최근 4개월 거리 그래프, 목표 대비 비교 그래프, 클릭형 Goals 차트, 월 이동 러닝 날짜 달력 표시
...
- 구현 기능: 총 뛴 거리, 러닝 횟수, 최근 러닝, 월간 거리, 최장 러닝 거리, 평균 페이스, 목표 설정, 주간·월간 목표 대비 진행률, D3 월간 거리 그래프, D3 목표 대비 비교 그래프, 클릭형 Goals 차트, 월 이동 달력 기반 러닝 날짜 표시.
...
- 웹 러닝 추적은 화면이 켜지고 브라우저가 foreground에 있는 동안의 기록을 우선 보장한다.
- iOS 앱은 native background location 브릿지 1차 구현을 완료했다. 실사용 안정성은 Xcode 실기기에서 위치 권한 `항상 허용`, 화면 잠금, 앱 전환 흐름으로 추가 검증한다.
...
- [x] 마이 페이지 Goals 섹션 클릭형 차트와 월 이동 달력 추가
- [x] Goals 요약 차트를 주간 목표·월간 목표·이번 주·이번 달 클릭형 차트로 개편
- [x] 하단 GNB를 텍스트 중심에서 아이콘 중심으로 변경
- [x] iPhone Apple MapKit 현재 위치 마커를 러너 SF Symbol로 커스터마이즈
- [x] 기록 탭 독립 페이지 전환: 지도 위 floating panel이 아니라 전체 기록 페이지로 표시
- [x] iPhone 기록 상세 경로를 Apple MapKit snapshot 이미지로 표시
- [x] NativeMap Capacitor plugin 초안 추가
- [ ] iPhone QA: 기록 페이지에서 여러 기록을 빠르게 눌러도 멈추지 않는지 확인
- [ ] iPhone QA: 백그라운드·화면 잠금 중 경로 기록이 이어지는지 확인
- [ ] iPhone QA: 하단 GNB 아이콘과 Goals 차트 전환 사용감 확인
